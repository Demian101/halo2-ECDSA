...
impl TestCircuitEcdsaVerifyConfig {
    ...
    pub fn config_range<N: PrimeField>(&self, layouter: &mut impl Layouter<N>) -> Result<(), Error> {
...
fn run<C: CurveAffine, N: PrimeField>() -> (TestCircuitEcdsaVerify<C, N>, Vec<Vec<N>>) {
    let g = C::generator();
...
fn run_fixed<C: CurveAffine, N: PrimeField>(
    ...
    let aux_generator = <Secp256k1 as CurveAffine>::CurveExt::random.to_affine();
    let circuit = TestCircuitEcdsaVerify::<Secp256k1, N> {
        public_key: Value::known(public_key.to_affine()),
        signature: Value::known((r, s)),
        msg_hash: Value::known(msg_hash),
        aux_generator,
        window_size: 2,
